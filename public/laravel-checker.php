...
$latestLaravelVersion = '12.0';

$laravelVersion = (isset($_GET['v'])) ? (string)$_GET['v'] : $latestLaravelVersion;

if (!in_array($laravelVersion, array('4.2', '5.0', '5.1', '5.2', '5.3', '5.4', '5.5', '5.6', '5.7', '5.8', '6.0', '7.0', '8.0', '9.0', '10.0', '11.0', '12.0'))) {
...
$requirements['session_enabled'] = extension_loaded("session");

// Filter PHP Extension
$requirements['filter_enabled'] = extension_loaded("filter");

// Hash PHP Extension
$requirements['hash_enabled'] = function_exists("hash");
    ...
    <div class="wrapper">
        ...
        <form action="?" method="get">
            <select name="v" onchange="this.form.submit()">
                <option value="12.0" <?php echo ($laravelVersion === '12.0') ? 'selected' : '' ?>>Laravel 12.0 - Latest</option>
                <option value="11.0" <?php echo ($laravelVersion === '11.0') ? 'selected' : '' ?>>Laravel 11.0</option>
                <option value="10.0" <?php echo ($laravelVersion === '10.0') ? 'selected' : '' ?>>Laravel 10.0</option>
                <option value="9.0" <?php echo ($laravelVersion === '9.0') ? 'selected' : '' ?>>Laravel 9.0</option>
        ...
        <?php if (isset($reqList[$laravelVersion]['session']) && $reqList[$laravelVersion]['session']) : ?>
            <p>Session PHP Extension <?php echo $requirements['session_enabled'] ? $strOk : $strFail; ?></p>
        <?php endif ?>

        <?php if (isset($reqList[$laravelVersion]['filter']) && $reqList[$laravelVersion]['filter']) : ?>
            <p>Filter PHP Extension <?php echo $requirements['filter_enabled'] ? $strOk : $strFail; ?></p>
        <?php endif ?>

        <?php if (isset($reqList[$laravelVersion]['hash']) && $reqList[$laravelVersion]['hash']) : ?>
            <p>Hash PHP Function <?php echo $requirements['hash_enabled'] ? $strOk : $strFail; ?></p>
        <?php endif ?>
